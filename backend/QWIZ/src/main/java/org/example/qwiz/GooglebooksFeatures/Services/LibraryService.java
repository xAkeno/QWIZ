            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                for (int i = 0; i < items.size(); i++) {
                    ...
                    for (int j = 0; j < authors.size(); j++) {
                        // Each element in the authors array is already a String, so we can get it
                        // directly
                    ...
                    LibraryDTO dto = new LibraryDTO(title, authorz[0], publishedDate, language, description, pageCount,
                            averageRating, thumbnailLink, pdfstatus, pdfLink, webread);
                    result.add(dto);
